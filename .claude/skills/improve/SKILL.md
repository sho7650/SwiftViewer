...
- `context7` — Phase 2: Official documentation lookup for SwiftUI and related tools
...
10. **Capture test baseline** — run unit tests to record the initial state:
   ```bash
   xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee .improvement-state/test-baseline.log
   BASELINE_UNIT_EXIT=$?
   ```
...
1. **Run with timeout** (if available — detected in Phase 0):
   ...
   ${TIMEOUT_CMD:-} ${TEST_TIMEOUT:-120}s xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/test-output.log
   TEST_EXIT=${PIPESTATUS[0]}
   ```
   If timeout is not available, run directly:
   ```bash
   xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/test-output.log
   TEST_EXIT=$?
   ```
...
3. **Parse failures** using xcodebuild output patterns (see `references/qa-guide.md` for parsing rules).
...
Parse test output using xcodebuild patterns (see `references/qa-guide.md`).
...
Use context7 to look up official documentation for SwiftUI APIs before flagging potential misuse.

#### Code Review (Claude)

If no SuperClaude `/sc:analyze` is available, perform manual code review:
- Read source files in SwiftViewer
...
4. **Verify the fix** by re-running only that test file:
   ```bash
   xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx -only-testing:SwiftViewerTests test 2>&1 2>&1 | tee /tmp/single-test-output.log
   SINGLE_TEST_EXIT=$?
   ```
...
```bash
xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/postfix-output.log
...
```bash
xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/phase3-precheck-output.log
...
```bash
xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/safety-unit-output.log
...
```bash
xcodebuild -project SwiftViewer.xcodeproj -scheme SwiftViewer -sdk macosx test 2>&1 2>&1 | tee /tmp/post-revert-output.log
